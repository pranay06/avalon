...
from connectors.quorum.quorum_wrapper \
    import QuorumWrapper as quorum_wrapper
...
class eth_cli:
    def __init__(self, config_file):
        ...
        if self.__config['quorum']['direct_registry_contract_file'] is None:
            raise Exception("Missing direct registry contract file path!!")
        if self.__config['quorum']['worker_registry_contract_file'] is None:
            raise Exception("Missing worker registry contract file path!!")
        self.__eth_client = quorum_wrapper(self.__config)

    def deploy_contracts(self):
        tcf_home = os.environ.get("TCF_HOME", "../../")
        self.deploy_solidity_contract(
            tcf_home + "/" +
            self.__config['quorum']['direct_registry_contract_file'])
        self.deploy_solidity_contract(
            tcf_home + "/" +
            self.__config['quorum']['worker_registry_contract_file'])
