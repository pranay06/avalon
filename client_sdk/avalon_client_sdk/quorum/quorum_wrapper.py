...
class QuorumWrapper():
    """
    Quorum wrapper class to interact with quorum blockchain
    ...
    def __init__(self, config):
        if self.__validate(config):
            ...
            # Quorum account address to interact with quorum network
            self.__eth_account_address = config['quorum']['eth_account']
            http_provider = config["quorum"]["eth_http_provider"]
            # Quorum http provider is endpoint to submit the transaction
            self.__w3 = Web3(HTTPProvider(http_provider))
            # Chain id signifies the which quorum network to use:
            # test net/main quorum network
            self.__channel_id = config["quorum"]["chain_id"]
            # Maximum amount of gas you’re willing to spend on
            # a particular transaction
            self.__gas_limit = config["quorum"]["gas_limit"]
            # Amount of Ether you’re willing to pay for every unit of gas
            self.__gas_price = config["quorum"]["gas_price"]
        else:
    ...
    def __validate(self, config):
        ...
        if os.environ["WALLET_PRIVATE_KEY"] is None:
            logging.error("Quorum account private key is not set!!")
            return False
        if config["quorum"]["eth_account"] is None:
            logging.error("Missing quorum account id!!")
            return False
        if config["quorum"]["chain_id"] is None:
            logging.error("Missing chain id in config!!")
            return False
        if config["quorum"]["eth_http_provider"] is None:
            logging.error("Missing quorum http provider url!!")
            return False
        if config["quorum"]["gas_limit"] is None:
            logging.error("Missing parameter gas limit")
            return False
        if config["quorum"]["gas_price"] is None:
    ...
    def deploy_contract(self, contract_interface):
        """
        Deploys solidity contract to quorum network identified by chain_id
        """
